...
// 2. Add Firestore Sync Logic after state declarations
const syncLogic = `
  // FIREBASE SYNC LOGIC
...
// Insert after role state declaration (around line 10-15)
// Let's insert it before the first useEffect in the component
content = content.replace(
    /useEffect\(\(\) => \{[\s\S]*?const timer = setInterval/,
    syncLogic + '\n  useEffect(() => {\n    const timer = setInterval'
);

// 3. Replace all state setters with our update functions
// We must be careful not to replace the useState declarations
content = content.replace(/setCategories\(/g, 'updateCategories(');
// Revert the useState declarations back
...
// Wait, the syncLogic code itself has `setCategories`! It should stay `setCategories` in the onSnapshot callback to prevent looping writes!
// Let's fix that. Since we already replaced `setCategories` with `updateCategories` globally, the syncLogic string above will ALSO have `setCategories` because it was a string literal! But wait, I'm appending it, and then doing replace.
// Ah, if I do global replace, it will replace inside `syncLogic` if `syncLogic` is already in `content`!
// Yes, `content = content.replace(..., syncLogic + ...)` -> then `content = content.replace(/setCategories.../g)`. So `syncLogic` will get modified!
// Let's write the string with `_setCategories_` and change it back later.

let finalContent = content.replace(/updateCategories/g, '_updateCategories_'); // just in case
// To avoid this issue, let's just do the replacements BEFORE inserting syncLogic.

fs.writeFileSync('e:/HOCHANH/EXE101/EXE101/CP3/smartstore-web/scratch/firebase-sync-temp.js', `
const fs = require('fs');
let content = fs.readFileSync('e:/HOCHANH/EXE101/EXE101/CP3/smartstore-web/src/App.jsx', 'utf8');
...
// Old: {id: 'all', label: 'TẤT CẢ'}, {id: 'coffee', label: 'CÀ PHÊ'}, {id: 'tea', label: 'TRÀ TRÁI CÂY'}, {id: 'smoothie', label: 'SINH TỐ'}, {id: 'food', label: 'ĐỒ ĂN'}
const oldCats = /\\[\\s*\\{id: 'all', label: 'TẤT CẢ'\\},[\\s\\S]*?\\{id: 'food', label: 'ĐỒ ĂN'\\},\\s*\\]/g;
const dynamicCats = \`[{id: 'all', name: 'TẤT CẢ'}, ...categories]\`;
// Wait, my replacement for the POS cats in the map should be careful because it expects cat.label or cat.name.
// In the previous code, it was cat.label. Our state categories have 'name'.
// Let's replace the whole map block for POS categories.

// We need to find the specific POS categories block.
// It looks like this:
/*
{[
  {id: 'all', label: 'TẤT CẢ'},
...
*/
content = content.replace(
    /\\{\\[[\\s\\S]*?\\{id: 'food', label: 'ĐỒ ĂN'\\},\\s*\\]\\.map\\(cat => \\([\\s\\S]*?\\{cat\\.label\\}[\\s\\S]*?<\\/button>\\s*\\)\\)\\}/,
